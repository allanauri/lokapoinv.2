...
module.exports = {
	...
	create: function(user){
        return knex('customer').insert({email_customer: user.email, fname_customer: user.fname, lname_customer: user.lname, password_customer: user.password, status: user.status}).then(id_customers => {
	...
	konfirmasi: function(id){
		return knex('customer').update('status',1).where('email_customer',id);
	}
